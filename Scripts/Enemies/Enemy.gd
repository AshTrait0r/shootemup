...
func get_damage() -> void:
	...
	if health == 0:
		...
		death_timer.start()
		used_letters.erase(prompt_text.substr(0, 1))
		#queue_free()
...
func set_next_character(next_character_index: int) -> String:
	var remaining_text = ""
	remaining_text = prompt_text.substr(next_character_index, prompt_text.length() - next_character_index)
	prompt.parse_bbcode(Prompt_list.set_center_tags(remaining_text))
